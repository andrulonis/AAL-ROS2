...
PUB_TOPIC_PARAM = "publisher_topic_name"
...
class LifecycleTalker(Node):
    ...
    def __init__(self, node_name, **kwargs):
        ...
        self.declare_parameter(PUB_TOPIC_PARAM, 'lifecycle_chatter')

        self.topic_name = self.get_parameter(PUB_TOPIC_PARAM).get_parameter_value().string_value
    ...
    def parameter_changed_callback(self, params):
        ...
        for param in params:
            if(param.name == PUB_TOPIC_PARAM and param.value != self.topic_name):
                self.topic_name = param.value
                old_activation = self._pub.is_activated
                self.destroy_lifecycle_publisher(self._pub)
                self._pub = self.create_lifecycle_publisher(std_msgs.msg.String, self.topic_name, 10)
                self._pub._enabled = old_activation
    ...
    def publish(self):
        ...
        msg.data = 'Node B Publishing' + str(self._count)
        self._count += 1
...
def main():
    ...
    lc_node = LifecycleTalker('publisher_B')
